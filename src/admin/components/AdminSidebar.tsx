...
const AdminSidebar: React.FC<SidebarProps> = ({ isCollapsed, onToggle }) => {
    ...
    const menuItems = [
        ...
        { icon: ShoppingCart, label: 'Órdenes', to: '/admin/' },
        { icon: FileText, label: 'Reportes', to: '/admin/' },
        { icon: Bell, label: 'Notificaciones', to: '/admin/' },
        { icon: Settings, label: 'Ajustes', to: '/admin/' },
        { icon: HelpCircle, label: 'Ayuda', to: '/admin/' },
    ];
    ...
    return (
        <div className={`bg-white border-r border-gray-200 transition-all duration-300 ease-in-out ${
            ...
            {/* Header */}
            <div className="p-4 border-b border-gray-200 flex items-center justify-between h-[5em]">
                {!isCollapsed && (
                    <CustomLogo/>
                )}
                <button
